// Template Source: BaseMethodCollectionPage.java.tt
// ------------------------------------------------------------------------------
// Copyright (c) Microsoft Corporation.  All Rights Reserved.  Licensed under the MIT License.  See License in the project root for license information.
// ------------------------------------------------------------------------------

package com.microsoft.graph.requests;

import com.microsoft.graph.http.IRequestBuilder;
import com.microsoft.graph.core.ClientException;

import java.util.Arrays;
import java.util.EnumSet;
import javax.annotation.Nullable;
import javax.annotation.Nonnull;

import com.microsoft.graph.http.BaseCollectionPage;
import com.microsoft.graph.requests.AuditEventGetAuditCategoriesCollectionRequestBuilder;
import com.microsoft.graph.requests.AuditEventGetAuditCategoriesCollectionPage;
import com.microsoft.graph.requests.AuditEventGetAuditCategoriesCollectionResponse;

// **NOTE** This file was generated by a tool and any changes will be overwritten.

/**
 * The class for the Audit Event Get Audit Categories Collection Page.
 */
public class AuditEventGetAuditCategoriesCollectionPage extends BaseCollectionPage<String, AuditEventGetAuditCategoriesCollectionRequestBuilder> {

    /**
     * A collection page for String.
     *
     * @param response The serialized AuditEventGetAuditCategoriesCollectionResponse from the service
     * @param builder The request builder for the next collection page
     */
    public AuditEventGetAuditCategoriesCollectionPage(@Nonnull final AuditEventGetAuditCategoriesCollectionResponse response, @Nonnull final AuditEventGetAuditCategoriesCollectionRequestBuilder builder) {
       super(response, builder);
    }

	/**
     * Creates the collection page for AuditEventGetAuditCategories
     *
     * @param pageContents       the contents of this page
     * @param nextRequestBuilder the request builder for the next page
     */
    public AuditEventGetAuditCategoriesCollectionPage(@Nonnull final java.util.List<String> pageContents, @Nullable final AuditEventGetAuditCategoriesCollectionRequestBuilder nextRequestBuilder) {
        super(pageContents, nextRequestBuilder);
    }
}
