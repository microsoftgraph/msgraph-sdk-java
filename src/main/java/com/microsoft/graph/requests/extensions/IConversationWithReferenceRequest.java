// ------------------------------------------------------------------------------
// Copyright (c) Microsoft Corporation.  All Rights Reserved.  Licensed under the MIT License.  See License in the project root for license information.
// ------------------------------------------------------------------------------

package com.microsoft.graph.requests.extensions;

import com.microsoft.graph.http.IRequestBuilder;
import com.microsoft.graph.core.ClientException;
import com.microsoft.graph.concurrency.ICallback;
import com.microsoft.graph.models.extensions.Conversation;
import com.microsoft.graph.requests.extensions.IConversationThreadCollectionRequestBuilder;
import com.microsoft.graph.requests.extensions.IConversationThreadRequestBuilder;
import java.util.Arrays;
import java.util.EnumSet;

import com.microsoft.graph.models.extensions.Conversation;
import com.microsoft.graph.http.IHttpRequest;
import com.microsoft.graph.serializer.IJsonBackedObject;


// **NOTE** This file was generated by a tool and any changes will be overwritten.

/**
 * The interface for the Conversation With Reference Request.
 */
public interface IConversationWithReferenceRequest extends IHttpRequest {

    void post(final Conversation newConversation, final IJsonBackedObject payload, final ICallback<? super Conversation> callback);

    Conversation post(final Conversation newConversation, final IJsonBackedObject payload) throws ClientException;

    void get(final ICallback<? super Conversation> callback);

    Conversation get() throws ClientException;

	void delete(final ICallback<? super Conversation> callback);

	void delete() throws ClientException;

	void patch(final Conversation sourceConversation, final ICallback<? super Conversation> callback);

	Conversation patch(final Conversation sourceConversation) throws ClientException;

    IConversationWithReferenceRequest select(final String value);

    IConversationWithReferenceRequest expand(final String value);

}
